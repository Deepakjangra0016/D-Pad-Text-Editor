...
main_application.title('D-Pad Text Editor')
...
text_editor.config(wrap='word', relief=tk.FLAT, yscrollcommand=scroll_bar.set)
...
def find_func(event=None):
    ...
    find_dialogue.geometry('400x200+400+200')
    find_dialogue.title('Find & Replace Text')
    ...
    find_button.grid(row=2, column=1, padx=8, pady=8)
    replace_button.grid(row=2, column=2, padx=8, pady=8)
...
view.add_checkbutton(label='Status Bar', onvalue=True, offvalue=False, image=status_bar_icon, compound=tk.LEFT, variable = show_statusbar, command= hide_statusbar)
